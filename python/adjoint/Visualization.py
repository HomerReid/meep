...
def plot_options(type='dft', plot3D=False):
    ...
    if type=='eps':
        options['cmap']=matplotlib.cm.Blues
        ...
        options['cmap']=matplotlib.cm.plasma
...
def visualize_sim(sim, fig=None, plot3D=None,
    ...
    ######################################################################
    # plot DFT cell regions, with index labels for flux cells
    ######################################################################
...
class PFCServer(object):
    ...
    def __init__(self, x, y, clist, cumulative=True,
        ...
        self.options       = options if options else plot_options(type='fields')
...
class AdjointVisualizer(object):
    ...
    def __init__(self, eps_options=None, cases=None, set_rcParams=True):
        self.geom_fig=self.ffd_fig=self.afd_fig=None
        #self.cases=cases if cases else self.ALL_CASES
        self.cases=['Geometry', 'ForwardFD']
        self.eps_options = eps_options if eps_options else plot_options('eps')
        self.plot_delay  = 0.1
        if set_rcParams:
            set_meep_rcParams()
        ...
        if 'ForwardFD' in self.cases:
            fig=plt.figure(2)
            ax=fig.gca(projection='3d')
            ax.set_title('Forward FD fields')
            self.ffd_fig=fig
    ...
    def update(self, sim, which):
        ...
        if which=='Geometry':
            plt.figure(1)
            self.geom_fig.clf()
            self.geom_fig.gca().set_title('Geometry')
            visualize_sim(sim,fig=self.geom_fig)

        if which=='ForwardFD':
            plt.figure(2)
            self.ffd_fig.clf()
            ax=fig.gca(projection='3d')
            self.ffd_fig.gca().set_title('Forward FD fields, t={:.3f}'.format(sim.round_time()))
            visualize_sim(sim,fig=self.ffd_fig, plot3D=True)

        elif which=='AdjointFD':
            plt.figure(3)
            self.afd_fig.clf()
            ax=fig.gca(projection='3d')
            self.afd_fig.gca().set_title('Adjoint FD fields, t={:.3f}'.format(sim.round_time()))
            visualize_sim(sim,fig=self.afd_fig, plot3D=True)
